...
# -- AgentCore config  ─────────────────────────────────────────────────────────
AGENTCORE_AGENT_ARN = os.getenv("AGENTCORE_AGENT_ARN", "")
AWS_REGION = os.getenv("AWS_REGION", "eu-west-1")

@st.cache_resource
def get_agentcore_client():
    # Usa la cadena estandar de resolucion de credenciales de boto3
    ...
    if credentials is None:
        raise RuntimeError(
            ...
            "Define AWS_ACCESS_KEY_ID y AWS_SECRET_ACCESS_KEY en .env "
            "o configura un perfil AWS valido."
        )

    return session.client("bedrock-agentcore")

def _decode_runtime_payload(response: dict) -> dict:
    """Extrae y parsea el payload devuelto por AgentCore Runtime."""
    raw = response.get("response")

    if raw is None:
        # Fallback defensivo por si cambia la clave en SDK/version.
        raw = response.get("payload") or response.get("output")

    if raw is None:
        return {"ok": False, "error": "No se encontro payload en la respuesta", "raw": str(response)}
    ...
    return {"ok": True, "respuesta": str(raw)}


def invoke_agent_core(payload: dict) -> dict:
    """
    Invoca el agente desplegado en Bedrock AgentCore Runtime.
    """
    try:
        if not AGENTCORE_AGENT_ARN:
            return {"ok": False, "error": "Falta AGENTCORE_AGENT_ARN en variables de entorno."}

        client = get_agentcore_client()
        payload_str = json.dumps(payload, ensure_ascii=False)
        print("[AGENTCORE] payload:", payload_str[:1500])
        ...
        return _decode_runtime_payload(response)
    except Exception as e:
        return {"ok": False, "error": f"Error al invocar el agente: {e}"}
...
with st.sidebar:
    ...
    idea_files = st.file_uploader(
        label="Archivos de idea del proyecto",
    ...
    if st.button("Generar planificación", type="primary", use_container_width=True):
        user_prompt = (
            f"Genera la planificacion del proyecto con hasta "
            f"{st.session_state.get('num_workers', 1)} trabajadores."
        )
        payload: dict = {
            "doc": st.session_state.get("idea_text", ""),
            "members": st.session_state.get("team_text", ""),
            "user_prompt": user_prompt,
        }

        with st.spinner("Consultando al modelo..."):
            try:
                plan_data = invoke_agent_core(payload)
